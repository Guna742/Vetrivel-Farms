...
function initThreeHero() {
  ...
  // Lighting (Natural Green)
  const ambientLight = new THREE.AmbientLight(0xffffff, 0.1)
  scene.add(ambientLight)

  const dirLight = new THREE.DirectionalLight(0x75D675, 0.3)
  dirLight.position.set(5, 5, 5)
  scene.add(dirLight)

  const dirLight2 = new THREE.DirectionalLight(0x75D675, 0.15)
...
function initSpotlightCards() {
  const cards = document.querySelectorAll('.alevior-card, .csr-card, .pillar-3d-card, .job-card, .career-project-card');
  cards.forEach(card => {
...
function initMagneticEffect() {
  ...
  cards.forEach(card => {
    card.addEventListener('mousemove', e => {
      ...
      // Move the card towards the mouse (30px max pull)
      gsap.to(card, {
        x: x * 0.4,
        y: y * 0.4,
        duration: 0.6,
